    ...
    fn step(
        ...
        match &mut state.computing {
            ...
            IterationState::Idle => {
                ...
                let todo_set = if context.filter_long_lived {
                    retain_long_lived(&context.graph, &todo.set)
                } else {
                    todo.set.clone()
                };

                if todo_set.is_empty() {
                    info!("Skipping short-lived set ({}).", log_set(&todo_set));
                    return Err(Working);
                }

                let todo = RemainingSet {
                    set: todo_set,
                    pivot_hint: todo.pivot_hint,
                };
