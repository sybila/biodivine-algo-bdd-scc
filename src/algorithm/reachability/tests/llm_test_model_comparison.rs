...
where
    VariantA: ReachabilityAlgorithm,
    VariantB: ReachabilityAlgorithm,
{
    // Load the model
    ...
    while !remaining_states.is_empty() {
        ...
        let sat_result = VariantA::compute((graph.clone(), initial_state.clone()))?;
        let bfs_result = VariantB::compute((graph.clone(), initial_state.clone()))?;
