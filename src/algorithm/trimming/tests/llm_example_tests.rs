...
fn test_trim_sources_from_empty_set() -> Cancellable<()> {
    ...
    let result = TrimSources::compute((graph.clone(), empty.clone()))?;
...
fn test_trim_sources_from_all_states() -> Cancellable<()> {
    ...
    let result = TrimSources::compute((graph.clone(), all.clone()))?;
    ...
    // relative to the new complement. Based on the log, result has 2 elements.
...
fn test_trim_sources_from_source_states_only() -> Cancellable<()> {
    ...
    let result = TrimSources::compute((graph.clone(), sources.clone()))?;
...
fn test_trim_sources_from_single_source_state() -> Cancellable<()> {
    ...
    let result = TrimSources::compute((graph.clone(), s011.clone()))?;
...
fn test_trim_sources_from_non_source_states() -> Cancellable<()> {
    ...
    let result = TrimSources::compute((graph.clone(), non_sources.clone()))?;
...
fn test_trim_sources_from_attractor_1() -> Cancellable<()> {
    ...
    let result = TrimSources::compute((graph.clone(), attractor_1.clone()))?;
...
fn test_trim_sources_from_attractor_2() -> Cancellable<()> {
    ...
    let result = TrimSources::compute((graph.clone(), attractor_2.clone()))?;
...
fn test_trim_sources_from_strong_basin_attr1() -> Cancellable<()> {
    ...
    let result = TrimSources::compute((graph.clone(), basin.clone()))?;
...
fn test_trim_sources_from_strong_basin_attr2() -> Cancellable<()> {
    ...
    let result = TrimSources::compute((graph.clone(), s101.clone()))?;
...
fn test_trim_sources_from_weak_basin() -> Cancellable<()> {
    ...
    let result = TrimSources::compute((graph.clone(), weak_basin.clone()))?;
...
fn test_trim_sources_from_mixed_set() -> Cancellable<()> {
    ...
    let result = TrimSources::compute((graph.clone(), mixed.clone()))?;
...
fn test_trim_sources_is_subset_of_initial() -> Cancellable<()> {
    ...
    // Trim sources always returns a subset of the initial set, and if the initial set is
    // a singleton, it is always trimmed.
    for state in ALL_STATES {
        let initial = mk_state(&graph, *state);
        let result = TrimSources::compute((graph.clone(), initial.clone()))?;
        assert!(
            result.is_empty(),
...
fn test_trim_sinks_from_empty_set() -> Cancellable<()> {
    ...
    let result = TrimSinks::compute((graph.clone(), empty.clone()))?;
...
fn test_trim_sinks_from_all_states() -> Cancellable<()> {
    ...
    let result = TrimSinks::compute((graph.clone(), all.clone()))?;
...
fn test_trim_sinks_from_sink_state_only() -> Cancellable<()> {
    ...
    let result = TrimSinks::compute((graph.clone(), s000.clone()))?;
...
fn test_trim_sinks_from_non_sink_states() -> Cancellable<()> {
    ...
    let result = TrimSinks::compute((graph.clone(), non_sinks.clone()))?;
...
fn test_trim_sinks_from_attractor_2() -> Cancellable<()> {
    ...
    let result = TrimSinks::compute((graph.clone(), attractor_2.clone()))?;
...
fn test_trim_sinks_from_source_states() -> Cancellable<()> {
    ...
    let result = TrimSinks::compute((graph.clone(), sources.clone()))?;
    ...
    // Then 001 and 010 become sinks relative to new complement, so they're added too.
    // This means all states in complement eventually get added, so result = empty.
    assert!(
        result.is_empty(),
...
fn test_trim_sinks_from_strong_basin_attr1() -> Cancellable<()> {
    ...
    let result = TrimSinks::compute((graph.clone(), basin.clone()))?;
    ...
    // Then 001 and 010 become sinks (they only reach 000 which is now in reachability set).
...
fn test_trim_sinks_from_strong_basin_attr2() -> Cancellable<()> {
    ...
    let result = TrimSinks::compute((graph.clone(), s101.clone()))?;
    ...
    // sinks get added iteratively, eventually all states in complement are added,
    // so result = empty.
    assert!(
        result.is_empty(),
...
fn test_trim_sinks_from_weak_basin() -> Cancellable<()> {
    ...
    let result = TrimSinks::compute((graph.clone(), weak_basin.clone()))?;

    // The algorithm removes sinks iteratively. With complement containing {000},
...
fn test_trim_sinks_from_mixed_set() -> Cancellable<()> {
    ...
    let result = TrimSinks::compute((graph.clone(), mixed.clone()))?;
...
fn test_trim_sinks_from_attractor_1_plus_basin() -> Cancellable<()> {
    ...
    let result = TrimSinks::compute((graph.clone(), initial.clone()))?;
...
fn test_trim_sinks_is_subset_of_initial() -> Cancellable<()> {
    ...
    // Trim sinks always returns a subset of the initial set
    for state in ALL_STATES {
        let initial = mk_state(&graph, *state);
        let result = TrimSinks::compute((graph.clone(), initial.clone()))?;
        assert!(
